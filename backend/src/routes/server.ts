...
router.post('/players/:uuid/teleport', async (req, res) => {
  ...
  try {
    ...
    client.send(JSON.stringify({ type: "teleport", target, x, y, z }));
});
console.log(`[Teleport] Sent teleport command for ${target} to ${x}, ${y}, ${z}`);
...
router.post('/update', (_req, res) => {
  exec('bash /home/minecraft-manager/minecraft-data/scripts/minecraft-update.sh', (err, stdout, stderr) => {
